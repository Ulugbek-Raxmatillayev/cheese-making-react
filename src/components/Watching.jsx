...
const Watching = ({ API,addToCard }) => {
    function scroll() {
        let cards = document.getElementById('card-flex')

        let leftScroll = document.getElementById('left')
        let rightScroll = document.getElementById('right')

        leftScroll.addEventListener('click', () => {
            cards.scrollLeft = -356
        })
        rightScroll.addEventListener('click' ,() => {
            cards.scrollRight  = -356
        })
    }
    ...
    return (
        <div className='container mx-auto my-[50px]'>
            <div className='flex flex-wrap justify-between  items-center'> 
                <div className="title">
                    <h2 className='text-2xl font-bold my-[20px]'>Просмотренные товары</h2>
                </div>
                <div className='flex flex-wrap justify-between gap-[20px]'>
                <button id='left' className='text-3xl'>{"<"}</button>
                <button id='right' className='text-3xl'>{">"}</button>
                </div>
            </div>
            <div id='card-flex' className="cardes grid grid-cols-6   gap-[120px] overflow-hidden">
                {
                    API.map((item) => {
                        return (
